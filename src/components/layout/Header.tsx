...
export function Header() {
  ...
  useEffect(() => {
    if (user) {
      supabase.from('user_roles').select('role').eq('user_id', user.id).in('role', ['moderator', 'admin']).maybeSingle().then(({
        data
      }) => {
        setIsModerator(!!data);
      });
        ...
        <div className="flex h-16 items-center justify-between">
          <div className="flex items-center gap-4">
            <a href="/" className="flex items-center gap-2">
              <div className="text-2xl font-bold text-primary">Benilton Freitas</div>
            </a>
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              <Button variant="outline" size="sm" onClick={() => setGeoModalOpen(true)} className="hidden lg:flex items-center gap-2">
                <MapPin className="h-4 w-4" />
                Região
              </Button>
              
              {user ? <UserMenu /> : <Button asChild size="sm">
                  <Link to="/auth">
          ...
          <Button variant="ghost" size="icon" className="md:hidden" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} aria-label={mobileMenuOpen ? "Fechar menu" : "Abrir menu"}>
        ...
        {mobileMenuOpen && <nav className="md:hidden py-4 border-t border-border">
            <div className="flex flex-col gap-4">
              ...
              <div className="border-l-2 border-primary pl-3">
                <Link to="/rede-pcd" className="text-sm font-semibold text-primary block mb-2">
                  Rede PcD
                </Link>
              ...
              {!user && <Button asChild size="sm" className="w-full">
                  <Link to="/auth">
                    <LogIn className="h-4 w-4 mr-2" />
                    Entrar
                  </Link>
